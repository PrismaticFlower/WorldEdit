...
TEST_CASE("commands_key_binder default key binds test", "[Commands]")
{
   key_input_manager key_input;
   commands commands;
   null_output_stream error_output;

   bool called = false;

   commands.add("test.command"s, [&]() { called = true; });

   commands_key_binder key_binder{commands, key_input, error_output};

   key_binder.set_default_bindings({
      {.command = "test.command"s, .binding = {.key = key::a}},
   });

   key_input.notify_key_down(key::a);
   key_input.update(false);

   REQUIRE(called);
}

TEST_CASE("commands_key_binder bind test", "[Commands]")
{
   key_input_manager key_input;
   commands commands;
   null_output_stream error_output;
   ...
   key_binder.bind("test.command"s, {.key = key::a}, {});
...
TEST_CASE("commands_key_binder unbind test", "[Commands]")
{
   key_input_manager key_input;
   commands commands;
   null_output_stream error_output;
   ...
   commands.add("test.command"s, [&]() { ++call_count; });
   ...
   key_input.notify_key_down(key::a);
   key_input.update(false);
   key_input.notify_key_up(key::a);

   REQUIRE(call_count == 1);

   key_binder.unbind({.key = key::a});
...
TEST_CASE("commands_key_binder bind over default test", "[Commands]")
{
   key_input_manager key_input;
   commands commands;
   null_output_stream error_output;
   ...
   commands.add("test.a"s, [&]() { a_called = true; });
   commands.add("test.b"s, [&]() { b_called = true; });

   commands_key_binder key_binder{commands, key_input, error_output};

   key_binder.set_default_bindings({
      {.command = "test.a"s, .binding = {.key = key::a}},
   });
   key_binder.bind("test.b"s, {.key = key::a}, {});
...
TEST_CASE("commands_key_binder default key binds no overwrite test",
          "[Commands]")
{
   key_input_manager key_input;
   commands commands;
   ...
   commands.add("test.set"s, [&]() { called = true; });
   commands.add("test.clear"s, [&]() { called = false; });

   commands_key_binder key_binder{commands, key_input, error_output};

   key_binder.bind("test.set"s, {.key = key::a}, {});
   key_binder.set_default_bindings(
      {{.command = "test.clear"s, .binding = {.key = key::a}}});
...
TEST_CASE("commands_key_binder bind missing command test", "[Commands]")
{
   key_input_manager key_input;
   commands commands;
   null_output_stream error_output;

   commands.add("test.command"s, [&]() {});

   commands_key_binder key_binder{commands, key_input, error_output};

   REQUIRE_THROWS_AS(key_binder.bind("test.command_preicse_typo"s, {.key = key::a}, {}),
                     unknown_command);
}

TEST_CASE("commands_key_binder bad command argument test", "[Commands]")
{
   key_input_manager key_input;
   commands commands;
   null_output_stream error_output;
   ...
   key_binder.bind("test.command one"s, {.key = key::a},
                   {}); // executing this will cause a invalid_command_argument exception

   key_input.notify_key_down(key::a);
   key_input.update(false); // but key_binder should catch it and print it out to error_output
