...
const we::world::world test_world_delete_animation_group_entry_broken_links = {
   .animation_groups = {
      world::entities_init,
      std::initializer_list{
         world::animation_group{
...
TEST_CASE("edits delete_animation_group_entry (broken links)", "[Edits]")
{
   world::world world = test_world_delete_animation_group_entry_broken_links;
   world::interaction_targets interaction_targets;
   world::edit_context edit_context{world, interaction_targets.creation_entity};

   auto edit = make_delete_animation_group_entry(&world.animation_groups[0].entries_broken_links,
                                                 1);

   edit->apply(edit_context);

   REQUIRE(world.animation_groups[0].entries_broken_links.size() == 2);
   CHECK(world.animation_groups[0].entries_broken_links[0].animation ==
         "Animation0");
   CHECK(world.animation_groups[0].entries_broken_links[1].animation ==
         "Animation2");

   edit->revert(edit_context);

   REQUIRE(world.animation_groups[0].entries_broken_links.size() == 3);
   CHECK(world.animation_groups[0].entries_broken_links[0].animation ==
         "Animation0");
   CHECK(world.animation_groups[0].entries_broken_links[1].animation ==
         "Animation1");
   CHECK(world.animation_groups[0].entries_broken_links[2].animation ==
         "Animation2");
}
}
