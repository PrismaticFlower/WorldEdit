...
{
   const float4 positionPS0 = input.flat_positionPS[0];
   const float4 positionPS1 = input.flat_positionPS[1];
   const float4 positionPS2 = input.flat_positionPS[2];
   ...
   return float4(input.color.rgb, line_alpha(min_distance));
}