...
void world_edit::update_ui() noexcept
{
   ...
   else if (_selection_edit_tool == selection_edit_tool::rotate_path) {
      ui_show_world_selection_rotate_path();
   }
   ...
   if (_animation_hierarchy_editor_open) {
      ui_show_animation_hierarchy_editor();
   }
   ...
   if (_block_material_editor_open) {
   ...
   if (_munge_manager_open) {
      ui_show_munge_manager();
   }

   if (_munge_config_editor_open) {
      ui_show_munge_config_editor();
   }
   ...
   if (_load_errors_open) {
      ui_show_load_errors();
   }

   if (_new_load_error_open) {
      ui_show_new_load_error();
   }
