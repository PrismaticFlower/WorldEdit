   ...
   : _device{device}, _max_sections{max_sections}, _max_timestamps{max_sections * 2}
{
   ...
   _readback_buffer = {_device.create_buffer(
                          {.size = _max_timestamps * sizeof(uint64) * gpu::frame_pipeline_length,
                           .debug_name = "Profiler Timestamp Readback Buffer"},
                          gpu::heap_type::readback),
                       _device.direct_queue};

   const uint64* const readback_buffer_address = static_cast<uint64*>(
      _device.map(_readback_buffer.get(), 0,
                  {0, _max_timestamps * sizeof(uint64) * gpu::frame_pipeline_length}));
...
void profiler::end(const uint32 index)
{
   ...
   section.command_list->query_timestamp(_query_heap.get(), section.end_timestamp_index);
}
...
void profiler::end_frame(gpu::compute_command_list& command_list)
{
   ...
   command_list.resolve_query_timestamp(_query_heap.get(), 0, timestamp_count,
                                        ...
                                        static_cast<uint32>(_max_timestamps *
                                                            sizeof(uint64) *
                                                            _device.frame_index()));
