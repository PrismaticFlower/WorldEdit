...
constexpr std::array billboard_patch_input_layout = {
   gpu::input_element_desc{.semantic_name = "POSITION",
                           .format = DXGI_FORMAT_R16G16B16A16_SINT,
                           .input_slot = 0},
   gpu::input_element_desc{.semantic_name = "NORMAL",
                              ...
                              const root_signature_library& root_signature_library)
{
   ...
   terrain_grid = {device.create_graphics_pipeline(
                      {.root_signature = root_signature_library.terrain.get(),
                       ...
                       .vs_bytecode = shader_library["terrain_patch_gridVS"sv],
   ...
   terrain_gradient_grid = {device.create_graphics_pipeline(
                               {.root_signature = root_signature_library.terrain.get(),

                                .vs_bytecode = shader_library["terrain_patch_gridVS"sv],
                                .ps_bytecode = shader_library["terrain_gradient_gridPS"sv],
                                ...
                                .debug_name = "terrain_gradient_grid"sv}),
                            device};
   ...
   block_custom_mesh_depth_prepass =
      {device.create_graphics_pipeline(
          {.root_signature = root_signature_library.block_custom_mesh.get(),

           .vs_bytecode = shader_library["block_custom_meshVS"sv],
           ...
           .debug_name = "block_custom_mesh_depth_prepass"sv}),
       device};

   block_custom_mesh_basic =
      {device.create_graphics_pipeline(
          {.root_signature = root_signature_library.block_custom_mesh.get(),
   ...
   block_custom_mesh_basic_lighting =
      {device.create_graphics_pipeline(
          {.root_signature = root_signature_library.block_custom_mesh.get(),
   ...
   block_custom_mesh_shadow =
      {device.create_graphics_pipeline(
          {.root_signature = root_signature_library.block_custom_mesh.get(),
           ...
           .debug_name = "block_custom_mesh_shadow"sv}),
       device};

   block_surface_highlight =
      {device.create_graphics_pipeline(
          {.root_signature = root_signature_library.block_surface_highlight.get(),

           .vs_bytecode = shader_library["block_surface_highlightVS"sv],
   ...
   billboard_patch_depth_prepass =
      {device.create_graphics_pipeline(
          {.root_signature = root_signature_library.billboard_patch.get(),

           .vs_bytecode = shader_library["billboard_patchVS"sv],
           .ps_bytecode = shader_library["billboard_patch_depth_cutoutPS"],

           .rasterizer_state = rasterizer_cull_none,
           .depth_stencil_state = depth_stencil_enabled,
           .input_layout = billboard_patch_input_layout,

           .dsv_format = DXGI_FORMAT_D32_FLOAT_S8X24_UINT,

           .debug_name = "billboard_patch_depth_prepass"sv}),
       device};

   billboard_patch_normal =
      {device.create_graphics_pipeline(
          {.root_signature = root_signature_library.billboard_patch.get(),

           .vs_bytecode = shader_library["billboard_patchVS"sv],
           .ps_bytecode = shader_library["billboard_patch_normalPS"sv],
           ...
           .debug_name = "billboard_patch_normal"sv}),
       device};

   billboard_patch_normal_transparent =
      {device.create_graphics_pipeline(
          {.root_signature = root_signature_library.billboard_patch.get(),
   ...
   billboard_patch_shadow =
      {device.create_graphics_pipeline(
          {.root_signature = root_signature_library.billboard_patch.get(),
           ...
           .debug_name = "billboard_patch_shadow"sv}),
       device};
   ...
   meta_draw_line_shape =
      {device.create_graphics_pipeline(
          {.root_signature = root_signature_library.meta_draw.get(),

           .vs_bytecode = shader_library["meta_draw_line_shapeVS"sv],
           .ps_bytecode = shader_library["meta_draw_linePS"sv],
           ...
           .debug_name = "meta_draw_line_shape"sv}),
       device};
