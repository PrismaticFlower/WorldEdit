...
private:
   ...
   static void destroy_resource(gpu_device& device, ID3D12Resource& resource);
};
...
struct resource_owner_base {
   ...
   ~resource_owner_base() requires gpu_device_child<T>
   {
      auto& derived = static_cast<T&>(*this);

      if (resource) {
         resource_destroyer_helper::destroy_resource(*derived.parent_gpu_device,
                                                     *derived.resource);
      }
   }
