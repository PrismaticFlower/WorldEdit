...
enum class ring_size_widget : uint8 {
   none,
...
struct gizmo_ring_size_state {
   gizmo_id id;
   quaternion rotation;

   state state = state::idle;
   ring_size_widget active_widget = ring_size_widget::none;

   float3 plane_positionWS;
   float3 active_positionWS;
   float3 activation_positionGS;
   float activation_inner_radius = 0.0f;
   float activation_outer_radius = 0.0f;
   float activation_radius = 0.0f;
   float activation_height = 0.0f;
...
#
   ...
   {
      gizmo_ring_size_state& gizmo = add_ring_size_gizmo(desc.name, desc.instance);

      gizmo.rotation = normalize(desc.gizmo_rotation);
      gizmo.alignment = desc.alignment;

      float ring_radius = inner_radius + outer_radius * 2.0f;
      ...
      if (not _orthographic_projection) {
         gizmo_camera_scale =
            distance(_camera_positionWS,
                     gizmo.rotation * float3{-ring_radius, 0.0f, 0.0f} + positionWS);
         gizmo_camera_scale = gizmo_camera_scale =
            std::min(gizmo_camera_scale,
                     distance(_camera_positionWS,
                              gizmo.rotation * float3{0.0f, -height, 0.0f} + positionWS));
         gizmo_camera_scale =
            std::min(gizmo_camera_scale,
                     distance(_camera_positionWS,
                              gizmo.rotation * float3{0.0f, 0.0f, -ring_radius} +
                                 positionWS));

         gizmo_camera_scale =
            std::min(gizmo_camera_scale,
                     distance(_camera_positionWS,
                              gizmo.rotation * float3{ring_radius, 0.0f, 0.0f} +
                                 positionWS));
         gizmo_camera_scale =
            std::min(gizmo_camera_scale,
                     distance(_camera_positionWS,
         ...
         gizmo_camera_scale =
            distance(_camera_positionWS,
                     gizmo.rotation * float3{-inner_radius, 0.0f, 0.0f} + positionWS);
         gizmo_camera_scale =
            std::min(gizmo_camera_scale,
                     distance(_camera_positionWS,
                              gizmo.rotation * float3{0.0f, 0.0f, -inner_radius} +
                                 positionWS));

         gizmo_camera_scale =
            std::min(gizmo_camera_scale,
                     distance(_camera_positionWS,
                              gizmo.rotation * float3{inner_radius, 0.0f, 0.0f} +
                                 positionWS));
         gizmo_camera_scale =
            std::min(gizmo_camera_scale,
                     distance(_camera_positionWS,
      ...
      if (gizmo.state == state::idle or gizmo.state == state::hovered) {
         ...
         const float3 x_negGS = float3{-ring_radius, 0.0f, 0.0f};
         const float3 y_negGS = float3{0.0f, -height, 0.0f};
         const float3 z_negGS = float3{0.0f, 0.0f, -ring_radius};

         const float3 x_posGS = float3{ring_radius, 0.0f, 0.0f};
         const float3 y_posGS = float3{0.0f, height, 0.0f};
         const float3 z_posGS = float3{0.0f, 0.0f, ring_radius};

         const float3 x_inner_negGS = float3{-inner_radius, 0.0f, 0.0f};
         const float3 z_inner_negGS = float3{0.0f, 0.0f, -inner_radius};

         const float3 x_inner_posGS = float3{inner_radius, 0.0f, 0.0f};
         const float3 z_inner_posGS = float3{0.0f, 0.0f, inner_radius};
         ...
         const float x_inner_neg_hit =
            iCappedCone(ray_originGS, ray_directionGS, x_inner_negGS,
                        x_inner_negGS + float3{gizmo_length, 0.0f, 0.0f},
                        gizmo_radius, 0.0f)
               .x;
         const float z_inner_neg_hit =
            iCappedCone(ray_originGS, ray_directionGS, z_inner_negGS,
                        z_inner_negGS + float3{0.0f, 0.0f, gizmo_length},
                        gizmo_radius, 0.0f)
               .x;

         const float x_inner_pos_hit =
            iCappedCone(ray_originGS, ray_directionGS, x_inner_posGS,
                        x_inner_posGS - float3{gizmo_length, 0.0f, 0.0f},
                        gizmo_radius, 0.0f)
               .x;
         const float z_inner_pos_hit =
            iCappedCone(ray_originGS, ray_directionGS, z_inner_posGS,
                        z_inner_posGS - float3{0.0f, 0.0f, gizmo_length},
                        gizmo_radius, 0.0f)
               .x;

         gizmo.active_widget = ring_size_widget::none;
         gizmo.state = state::idle;
         ...
         if (x_neg_hit > 0.0f and x_neg_hit < nearest) {
            nearest = x_neg_hit;
            gizmo.state = state::hovered;
            gizmo.active_widget = ring_size_widget::x_neg;
         }

         if (y_neg_hit > 0.0f and y_neg_hit < nearest) {
            nearest = y_neg_hit;
            gizmo.state = state::hovered;
            gizmo.active_widget = ring_size_widget::y_neg;
         }

         if (z_neg_hit > 0.0f and z_neg_hit < nearest) {
            nearest = z_neg_hit;
            gizmo.state = state::hovered;
            gizmo.active_widget = ring_size_widget::z_neg;
         }

         if (x_pos_hit > 0.0f and x_pos_hit < nearest) {
            nearest = x_pos_hit;
            gizmo.state = state::hovered;
            gizmo.active_widget = ring_size_widget::x_pos;
         }

         if (y_pos_hit > 0.0f and y_pos_hit < nearest) {
            nearest = y_pos_hit;
            gizmo.state = state::hovered;
            gizmo.active_widget = ring_size_widget::y_pos;
         }

         if (z_pos_hit > 0.0f and z_pos_hit < nearest) {
            nearest = z_pos_hit;
            gizmo.state = state::hovered;
            gizmo.active_widget = ring_size_widget::z_pos;
         }

         if (x_inner_neg_hit > 0.0f and x_inner_neg_hit < nearest) {
            nearest = x_inner_neg_hit;
            gizmo.state = state::hovered;
            gizmo.active_widget = ring_size_widget::x_inner_neg;
         }

         if (z_inner_neg_hit > 0.0f and z_inner_neg_hit < nearest) {
            nearest = z_inner_neg_hit;
            gizmo.state = state::hovered;
            gizmo.active_widget = ring_size_widget::z_inner_neg;
         }

         if (x_inner_pos_hit > 0.0f and x_inner_pos_hit < nearest) {
            nearest = x_inner_pos_hit;
            gizmo.state = state::hovered;
            gizmo.active_widget = ring_size_widget::x_inner_pos;
         }

         if (z_inner_pos_hit > 0.0f and z_inner_pos_hit < nearest) {
            nearest = z_inner_pos_hit;
            gizmo.state = state::hovered;
            gizmo.active_widget = ring_size_widget::z_inner_pos;
         }
      ...
      if (_input.left_mouse_down) {
         if (gizmo.state == state::idle) {
            ...
            gizmo.plane_positionWS = ray_originWS + ray_directionWS * _closest_gizmo;
            gizmo.active_positionWS = positionWS;
            gizmo.activation_inner_radius = inner_radius;
            gizmo.activation_outer_radius = outer_radius;
            gizmo.activation_radius = ring_radius;
            gizmo.activation_height = height;
            ...
            case ring_size_widget::x_pos:
            case ring_size_widget::y_pos:
            case ring_size_widget::z_pos:
            case ring_size_widget::x_inner_neg:
            case ring_size_widget::z_inner_neg:
            case ring_size_widget::x_inner_pos:
            case ring_size_widget::z_inner_pos: {
               const float3 eye_directionGS =
                  conjugate(gizmo.rotation) *
                  (gizmo.plane_positionWS - _camera_positionWS);
               float3 axisGS = {};

               if (gizmo.active_widget == ring_size_widget::x_neg or
                   gizmo.active_widget == ring_size_widget::x_pos or
                   gizmo.active_widget == ring_size_widget::x_inner_neg or
                   gizmo.active_widget == ring_size_widget::x_inner_pos) {
                  axisGS = float3{1.0f, 0.0f, 0.0f};
               }
               else if (gizmo.active_widget == ring_size_widget::y_neg or
                        gizmo.active_widget == ring_size_widget::y_pos) {
                  axisGS = float3{0.0f, 1.0f, 0.0f};
               }
               else if (gizmo.active_widget == ring_size_widget::z_neg or
                        gizmo.active_widget == ring_size_widget::z_pos or
                        gizmo.active_widget == ring_size_widget::z_inner_neg or
                        gizmo.active_widget == ring_size_widget::z_inner_pos) {
                  axisGS = float3{0.0f, 0.0f, 1.0f};
               }
      ...
      if (gizmo.state == state::active) {
         ...
         if (const float hit = intersect_plane(ray_originGS, ray_directionGS, planeGS);
            ...
            else if (gizmo.active_widget == ring_size_widget::y_neg) {
               axisGS = {0.0f, -1.0f, 0.0f};
            }
            else if (gizmo.active_widget == ring_size_widget::z_neg or
                     gizmo.active_widget == ring_size_widget::z_inner_neg) {
            ...
            else if (gizmo.active_widget == ring_size_widget::x_pos or
                     gizmo.active_widget == ring_size_widget::x_inner_pos) {
               axisGS = {1.0f, 0.0f, 0.0f};
            }
            else if (gizmo.active_widget == ring_size_widget::y_pos) {
               axisGS = {0.0f, 1.0f, 0.0f};
            }
            else if (gizmo.active_widget == ring_size_widget::z_pos or
                     gizmo.active_widget == ring_size_widget::z_inner_pos) {
               axisGS = {0.0f, 0.0f, 1.0f};
            }
            ...
            if (gizmo.active_widget == ring_size_widget::x_neg or
                gizmo.active_widget == ring_size_widget::x_pos or
                gizmo.active_widget == ring_size_widget::z_neg or
                gizmo.active_widget == ring_size_widget::z_pos) {
               offset = std::max(-gizmo.activation_inner_radius, offset);
            }
            else if (gizmo.active_widget == ring_size_widget::y_neg or
                     gizmo.active_widget == ring_size_widget::y_pos) {
               offset = std::max(-gizmo.activation_height, offset);
            }
            else if (gizmo.active_widget == ring_size_widget::x_inner_neg or
                     gizmo.active_widget == ring_size_widget::x_inner_pos or
                     gizmo.active_widget == ring_size_widget::z_inner_neg or
                     gizmo.active_widget == ring_size_widget::z_inner_pos) {
               offset = std::max(-gizmo.activation_radius, offset);
            }

            float3 new_positionGS = positionGS;

            if (gizmo.active_widget == ring_size_widget::x_neg) {
               inner_radius = gizmo.activation_inner_radius + offset;
               new_positionGS.x -= offset;
            }
            else if (gizmo.active_widget == ring_size_widget::y_neg) {
               height = gizmo.activation_height + offset;
               new_positionGS.y -= offset;
            }
            else if (gizmo.active_widget == ring_size_widget::z_neg) {
               inner_radius = gizmo.activation_inner_radius + offset;
               new_positionGS.z -= offset;
            }
            else if (gizmo.active_widget == ring_size_widget::x_pos) {
               inner_radius = gizmo.activation_inner_radius + offset;
               new_positionGS.x += offset;
            }
            else if (gizmo.active_widget == ring_size_widget::y_pos) {
            ...
            else if (gizmo.active_widget == ring_size_widget::z_pos) {
               inner_radius = gizmo.activation_inner_radius + offset;
               new_positionGS.z += offset;
            }
            else if (gizmo.active_widget == ring_size_widget::x_inner_neg or
                     ...
                     gizmo.active_widget == ring_size_widget::x_inner_pos or
                     gizmo.active_widget == ring_size_widget::z_inner_pos) {
               outer_radius = std::max((gizmo.activation_radius - offset -
                                        gizmo.activation_inner_radius) /
                                          2.0f,
                                       0.0f);
               inner_radius = std::max(gizmo.activation_inner_radius + offset, 0.0f);
            }
      ...
      // Update ring_radius after edits.
      ring_radius = inner_radius + outer_radius * 2.0f;

      {

         const bool is_hover = gizmo.state == state::hovered;
         const bool is_active = gizmo.state == state::active;
         const bool x_neg_active = gizmo.active_widget == ring_size_widget::x_neg;
         const bool y_neg_active = gizmo.active_widget == ring_size_widget::y_neg;
         const bool z_neg_active = gizmo.active_widget == ring_size_widget::z_neg;
         const bool x_pos_active = gizmo.active_widget == ring_size_widget::x_pos;
         const bool y_pos_active = gizmo.active_widget == ring_size_widget::y_pos;
         const bool z_pos_active = gizmo.active_widget == ring_size_widget::z_pos;
         const bool x_inner_neg_active =
            gizmo.active_widget == ring_size_widget::x_inner_neg;
         const bool z_inner_neg_active =
            gizmo.active_widget == ring_size_widget::z_inner_neg;
         const bool x_inner_pos_active =
            gizmo.active_widget == ring_size_widget::x_inner_pos;
         const bool z_inner_pos_active =
            gizmo.active_widget == ring_size_widget::z_inner_pos;
         ...
         const float3 x_negWS = positionWS - x_axisWS * ring_radius;
         const float3 y_negWS = positionWS - y_axisWS * height;
         const float3 z_negWS = positionWS - z_axisWS * ring_radius;

         const float3 x_posWS = positionWS + x_axisWS * ring_radius;
         const float3 y_posWS = positionWS + y_axisWS * height;
         const float3 z_posWS = positionWS + z_axisWS * ring_radius;

         const float3 x_inner_negWS = positionWS - x_axisWS * inner_radius;
         const float3 z_inner_negWS = positionWS - z_axisWS * inner_radius;

         const float3 x_inner_posWS = positionWS + x_axisWS * inner_radius;
         const float3 z_inner_posWS = positionWS + z_axisWS * inner_radius;
         ...
         if (is_active) {
            ...
            if (x_inner_neg_active) {
               draw_lists.cones.emplace_back(x_inner_negWS,
                                             x_inner_negWS + x_axisWS * gizmo_length,
                                             gizmo_radius, x_color_hover);
            }

            if (z_inner_neg_active) {
               draw_lists.cones.emplace_back(z_inner_negWS,
                                             z_inner_negWS + z_axisWS * gizmo_length,
                                             gizmo_radius, z_color_hover);
            }

            if (x_inner_pos_active) {
               draw_lists.cones.emplace_back(x_inner_posWS,
                                             x_inner_posWS - x_axisWS * gizmo_length,
                                             gizmo_radius, x_color_hover);
            }

            if (z_inner_pos_active) {
               draw_lists.cones.emplace_back(z_inner_posWS,
                                             z_inner_posWS - z_axisWS * gizmo_length,
                                             gizmo_radius, z_color_hover);
            }
         }
         else {
            ...
            draw_lists.cones.emplace_back(x_inner_negWS,
                                          x_inner_negWS + x_axisWS * gizmo_length,
                                          gizmo_radius,
                                          is_hover and x_inner_neg_active ? x_color_hover
                                                                          : x_color);

            draw_lists.cones.emplace_back(x_inner_posWS,
                                          x_inner_posWS - x_axisWS * gizmo_length,
                                          gizmo_radius,
                                          is_hover and x_inner_pos_active ? x_color_hover
                                                                          : x_color);

            draw_lists.cones.emplace_back(z_inner_negWS,
                                          z_inner_negWS + z_axisWS * gizmo_length,
                                          gizmo_radius,
                                          is_hover and z_inner_neg_active ? z_color_hover
                                                                          : z_color);

            draw_lists.cones.emplace_back(z_inner_posWS,
                                          z_inner_posWS - z_axisWS * gizmo_length,
                                          gizmo_radius,
                                          is_hover and z_inner_pos_active ? z_color_hover
                                                                          : z_color);
         }
      }
...
private:
   ...
   std::vector<gizmo_ring_size_state> _ring_size_gizmos;
   ...
   auto add_ring_size_gizmo(const std::string_view name, const int64 instance) noexcept
      -> gizmo_ring_size_state&
   {
      for (gizmo_ring_size_state& existing : _ring_size_gizmos) {
         if (existing.id.name == name and existing.id.instance == instance) {
            existing.submitted_last_frame = true;
      ...
      return _ring_size_gizmos.emplace_back(gizmo_ring_size_state{
         .id = {.name = std::string{name}, .instance = instance}});
   }
   ...
   void garbage_collect() noexcept
   {
      ...
      for (auto it = _ring_size_gizmos.begin(); it != _ring_size_gizmos.end();) {
         gizmo_ring_size_state& gizmo = *it;

         if (not gizmo.submitted_last_frame) {
            it = _ring_size_gizmos.erase(it);
         }
         else {
            gizmo.submitted_last_frame = false;
...
bool gizmos::gizmo_ring_size(const gizmo_ring_size_desc& desc,
                             float3& positionWS, float& inner_radius,
                             float& outer_radius, float& height) noexcept
{
   return impl->gizmo_ring_size(desc, positionWS, inner_radius, outer_radius, height);
}
