...
struct detail::bvh_impl {
            ...
            for (int32 i = node->left_child_or_first_tri; i < last_tri; ++i) {
               ...
               if (float hit = 0.0f;
                  ...
                  if (flags.accept_first_hit) {
                     return std::optional{bvh::ray_hit{.distance = closest_hit,
                                                       .normal = hit_normal}};
                  }
               }
            }
   ...
   {
      return _nodes[_root_node_index].bbox;
   }
...
struct detail::top_level_bvh_impl {
      ...
      : _instances{instances}
   {
      ...
      _nodes.resize(instances.size() * 2 - 1);

      _nodes[_root_node_index] = {.instance_count =
                                     static_cast<int32>(instances.size())};
      ...
      -> std::optional<float>
   {
      ...
      while (stack_ptr >= 0) {
         ...
         if (node->is_leaf()) {
            const int32 last_tri =
               node->left_child_or_first_instance + node->instance_count;

            for (int32 i = node->left_child_or_first_instance; i < last_tri; ++i) {
               ...
               if (std::optional<bvh::ray_hit> hit =
                  ...
                  if (flags.accept_first_hit) return std::optional{closest_hit};
               }
            }
         }
         else {
            int32 child_index0 = node->left_child_or_first_instance;
            int32 child_index1 = node->left_child_or_first_instance + 1;

            float child_distance0 = 0.0f;
            float child_distance1 = 0.0f;

            const bool hit0 = intersect_aabb(ray_originWS, inv_ray_directionWS,
                                             _nodes[child_index0].bbox,
                                             closest_hit, child_distance0);
            const bool hit1 = intersect_aabb(ray_originWS, inv_ray_directionWS,
                                             _nodes[child_index1].bbox,
                                             closest_hit, child_distance1);
...
private:
   struct node {
      math::bounding_box bbox;
      int32 left_child_or_first_instance = 0;
   ...
   void update_node_bounds(const int32 node_index) noexcept
   {
      ...
      for (int32 i = node.left_child_or_first_instance; i < last_instance; ++i) {
         const instance& instance = _instances[_index[i]];
   ...
   void subdivide(const int32 node_index, std::span<const float3> centroids) noexcept
   {
      ...
      int32 i = node.left_child_or_first_instance;
      ...
      const int32 left_count = i - node.left_child_or_first_instance;

      if (left_count == 0 or left_count == node.instance_count) return;
      ...
      _nodes[left_child_index] = {.left_child_or_first_instance =
                                     node.left_child_or_first_instance,
                                  .instance_count = left_count};
      _nodes[right_child_index] = {.left_child_or_first_instance = i,
                                   .instance_count = node.instance_count - left_count};

      node.left_child_or_first_instance = left_child_index;
                              ...
                              float& best_split_cost) const noexcept
   {
      for (float3_axis axis : {&float3::x, &float3::y, &float3::z}) {
         ...
         const int32 last_tri = node.left_child_or_first_instance + node.instance_count;
