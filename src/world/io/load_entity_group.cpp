   ...
   try {
      ...
      for (auto& key_node : assets::config::read_config(entity_group_data)) {
         if (string::iequals(key_node.key, "Object"sv)) {
            object& object = group.objects.emplace_back();
            ...
            light& light = group.lights.emplace_back();

            light.name = key_node.values.get<std::string>(0);

            for (auto& light_prop : key_node) {
               if (string::iequals(light_prop.key, "Rotation"sv)) {
                  ...
                  default:
                     output.write(
                        "Warning! World light '{}' has invalid light type! "
                  ...
                  if (light_prop.values.size() >= 2) {
                     switch (const int addressing = light_prop.values.get<int>(1);
                             addressing) {
                     case 0:
                     case 1:
                     ...
                     default:
                        output.write(
                           "Warning! World light '{}' has invalid texture "
