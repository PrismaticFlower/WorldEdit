...
struct shadow_silouhette_edge_index {
   uint32 vertex_index = 0;
   std::array<std::array<uint8, 3>, 2> normalSS = {};

   bool operator==(const shadow_silouhette_edge_index&) const noexcept = default;

   template<typename H>
   friend H AbslHashValue(H h, const shadow_silouhette_edge_index& cached)
   {
      return H::combine(std::move(h), cached.vertex_index, cached.normalSS);
   }
};
   ...
   -> std::vector<shadow_silouhette_edge>
{
   ...
   for (const shadow_face& face : mesh.faces) {
      for (uint32 edge_index = face.first_edge;;) {
         ...
         if (mesh.bone_indices) {
            if (face.normalSS == twin_face.normalSS) {
               if (mesh.bone_weights) {
                  const std::array<uint8, 3> bone_weights =
                     mesh.bone_weights[edge.vertex];
                  const std::array<uint8, 3> bone_indices =
                     mesh.bone_indices[edge.vertex];

                  bool all_vertices_match = true;

                  for (uint32 next_edge_index = face.first_edge;;) {
                     const half_edge& iter_edge = mesh.half_edges[next_edge_index];

                     all_vertices_match &=
                        bone_weights == mesh.bone_weights[iter_edge.vertex];
                     all_vertices_match &=
                        bone_indices == mesh.bone_indices[iter_edge.vertex];

                     if (not all_vertices_match) break;

                     next_edge_index = iter_edge.next_edge;

                     if (next_edge_index == face.first_edge) break;
                  }

                  if (not all_vertices_match) goto add_edge;

                  for (uint32 next_edge_index = twin_face.first_edge;;) {
                     const half_edge& iter_edge = mesh.half_edges[next_edge_index];
                     ...
                     if (next_edge_index == twin_face.first_edge) break;
                  }

                  if (all_vertices_match) goto next_edge;
               }
               else {
                  const uint8 bone_index = mesh.bone_indices[edge.vertex][0];
                  bool all_vertices_match = true;
                  ...
                  for (uint32 next_edge_index = face.first_edge;;) {
                     ...
                     all_vertices_match &=
                        bone_index == mesh.bone_indices[iter_edge.vertex][0];
            ...
            if (face.normalSS == twin_face.normalSS) goto next_edge;
         }
   ...
   absl::flat_hash_map<shadow_silouhette_edge_index, uint32> forward_connection_map;
   forward_connection_map.reserve(silouhette_edges.size());

   for (std::size_t i = 0; i < silouhette_edges.size(); ++i) {
      const shadow_silouhette_edge& edge = silouhette_edges[i];

      forward_connection_map
         .insert_or_assign({.vertex_index = mesh.half_edges[edge.half_edges[0]].vertex,
                            .normalSS = edge.normalSS},
                           static_cast<uint32>(i));
   }

   absl::flat_hash_map<shadow_silouhette_edge_index, uint32> back_connection_map;
   back_connection_map.reserve(silouhette_edges.size());

   for (std::size_t i = 0; i < silouhette_edges.size(); ++i) {
      const shadow_silouhette_edge& edge = silouhette_edges[i];

      back_connection_map
         .insert_or_assign({.vertex_index = mesh.half_edges[edge.half_edges[1]].vertex,
                            .normalSS = edge.normalSS},
                           static_cast<uint32>(i));
   }

   visited_edges.clear();
   visited_edges.resize(silouhette_edges.size());

   std::vector<shadow_silouhette_edge> reduced_silouhette_edges;
   reduced_silouhette_edges.reserve(silouhette_edges.size());
   ...
   for (std::size_t i = 0; i < silouhette_edges.size(); ++i) {
      ...
      shadow_silouhette_edge edge = silouhette_edges[i];

      forward_connection_map.erase(
         {.vertex_index = mesh.half_edges[edge.half_edges[0]].vertex,
          .normalSS = edge.normalSS});
      back_connection_map.erase(
         {.vertex_index = mesh.half_edges[edge.half_edges[1]].vertex,
          .normalSS = edge.normalSS});

      for (auto next_it = forward_connection_map.find(
              {.vertex_index = mesh.half_edges[edge.half_edges[1]].vertex,
               .normalSS = edge.normalSS});
           next_it != forward_connection_map.end();
           next_it = forward_connection_map.find(
              {.vertex_index = mesh.half_edges[edge.half_edges[1]].vertex,
               .normalSS = edge.normalSS})) {
         const uint32 merged_candidate_index = next_it->second;

         if (visited_edges[merged_candidate_index]) break;

         const shadow_silouhette_edge& merge_candidate_edge =
            silouhette_edges[merged_candidate_index];

         const std::array<uint32, 3> tri = {
            mesh.half_edges[edge.half_edges[0]].vertex,
            mesh.half_edges[edge.half_edges[1]].vertex,
            mesh.half_edges[merge_candidate_edge.half_edges[1]].vertex,
         };

         const float3& v0 = mesh.positionSS[tri[0]];
         const float3& v1 = mesh.positionSS[tri[1]];
         const float3& v2 = mesh.positionSS[tri[2]];

         // Check if the edges are colinear.
         if (cross(v1 - v0, v2 - v0) != float3{0.0f, 0.0f, 0.0f}) {
            break;
         }

         if (mesh.bone_indices) {
            if (mesh.bone_weights) {
               if (mesh.bone_weights[tri[0]] != mesh.bone_weights[tri[1]] or
                   mesh.bone_weights[tri[0]] != mesh.bone_weights[tri[2]]) {
                  break;
               }

               if (mesh.bone_indices[tri[0]] != mesh.bone_indices[tri[1]] or
                   mesh.bone_indices[tri[0]] != mesh.bone_indices[tri[2]]) {
                  break;
               }
            }
            else {
               if (mesh.bone_indices[tri[0]][0] != mesh.bone_indices[tri[1]][0] or
                   mesh.bone_indices[tri[0]][0] != mesh.bone_indices[tri[2]][0]) {
                  break;
               }
            }
         }

         visited_edges[merged_candidate_index] = true;
         edge.half_edges[1] = merge_candidate_edge.half_edges[1];
         forward_connection_map.erase(next_it);
      }

      for (auto back_it = back_connection_map.find(
              {.vertex_index = mesh.half_edges[edge.half_edges[0]].vertex,
               .normalSS = edge.normalSS});
           back_it != back_connection_map.end();
           back_it = back_connection_map.find(
              {.vertex_index = mesh.half_edges[edge.half_edges[0]].vertex,
               .normalSS = edge.normalSS})) {
         const uint32 merged_candidate_index = back_it->second;
         ...
         const std::array<uint32, 3> tri = {
            mesh.half_edges[merge_candidate_edge.half_edges[0]].vertex,
            mesh.half_edges[edge.half_edges[0]].vertex,
            mesh.half_edges[edge.half_edges[1]].vertex,
         };
         ...
         visited_edges[merged_candidate_index] = true;
         edge.half_edges[0] = merge_candidate_edge.half_edges[0];
         back_connection_map.erase(back_it);
      }

      reduced_silouhette_edges.push_back(edge);
   }

   return reduced_silouhette_edges;
}
